...
{
    static void AddBook(Library library, string[] bookParams)
    {
    ...
    static void Main(string[] args)
    {
        ...
        using (StreamReader reader = new StreamReader("../../Test.txt"))
        {
            ...
            for (int i = 0; i < numberOfBooks; i++)
            {
                ...
                AddBook(library, bookParams);
            }

            startDate = DateTime.ParseExact(reader.ReadLine(), "dd.MM.yyyy", CultureInfo.InvariantCulture);
        }

        File.WriteAllText("../../Output.txt",
            string.Join(Environment.NewLine, 
            library.Books.Where(b => b.ReleaseDate > startDate).
            OrderBy(b => b.ReleaseDate).
            ThenBy(b => b.Title).
            Select(book => $"{book.Title} -> {book.ReleaseDate:dd.MM.yyy}")));
    }
}