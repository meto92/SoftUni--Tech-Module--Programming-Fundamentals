...
class BookLibraryModification
{
    static void Main(string[] args)
    {
        ...
        {
            string[] bookParams = Console.ReadLine().Split(' ');
        ...
        foreach (Book book in library.Books.Where(b => b.ReleaseDate > startDate).OrderBy(b => b.ReleaseDate).ThenBy(b => b.Title))
        {
            string title = book.Title;
            DateTime releaseDate = book.ReleaseDate;

            Console.WriteLine($"{title} -> {releaseDate:dd.MM.yyy}");
        }
    }
}